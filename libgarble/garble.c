...
{
    //printf("In garble half gates\n");
    size_t nxors = 0;
    for (size_t i = 0; i < gc->q; i++) {
        garble_gate *g = &gc->gates[i];
        bool isxor = g->type == GARBLE_GATE_XOR;
        nxors += isxor ? 1 : 0;
        //printf("%ld %ld %ld %d \n\n", gc->gates[i].input0, gc->gates[i].input1, gc->gates[i].output, gc->gates[i].type);

        // uint16_t *val = (uint16_t*) &(gc->wires[2 * g->input0]);
        // char buffer[100];

        // sprintf(buffer,"%i%i%i%i%i%i%i%i",val[0], val[1], val[2], val[3], val[4], val[5], val[6], val[7]);
        //printf("%s\n", buffer);

        // val = (uint16_t*) &(gc->wires[2 * g->input0 + 1]);
        // sprintf(buffer,"%i%i%i%i%i%i%i%i",val[0], val[1], val[2], val[3], val[4], val[5], val[6], val[7]);
        //printf("%s\n", buffer);

        // val = (uint16_t*) &(gc->wires[2 * g->input1]);

        // sprintf(buffer,"%i%i%i%i%i%i%i%i",val[0], val[1], val[2], val[3], val[4], val[5], val[6], val[7]);
        //printf("%s\n", buffer);

        // val = (uint16_t*) &(gc->wires[2 * g->input1 + 1]);
        // sprintf(buffer,"%i%i%i%i%i%i%i%i",val[0], val[1], val[2], val[3], val[4], val[5], val[6], val[7]);
        //printf("%s\n", buffer);
              ...
              block *restrict output_labels)
{
    ...
    if (input_labels) {
        ...
        for (uint64_t i = 0; i < gc->n; ++i) {
          ...
          //   uint16_t *val = (uint16_t*) &(gc->wires[2 * i]);
          //  char buffer[100];

          //  sprintf(buffer,"%i%i%i%i%i%i%i%i",val[0], val[1], val[2], val[3], val[4], val[5], val[6], val[7]);
           //printf("%s\n", buffer);

          //  val = (uint16_t*) &(gc->wires[2 * i + 1]);
          //  sprintf(buffer,"%i%i%i%i%i%i%i%i",val[0], val[1], val[2], val[3], val[4], val[5], val[6], val[7]);
           //printf("%s\n", buffer);
        }
    }
