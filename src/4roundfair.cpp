...
int garbler(char *ip){
  ...
  char commitP1P2[2][SHA256_DIGEST_LENGTH];
  char cr12[2][SHA256_DIGEST_LENGTH], c21[SHA256_DIGEST_LENGTH];
  block open12[2];
  block g_op[2];
  ...
  if(buffer[1]=='A'){
    ...
    //commiting on some random input for proof
      open12[id]=garble_random_block();
      commit(&(commitP1P2[id][0]),open12[id],NULL, COMMIT_SCHEME_SHA256);
  ...
  //Sharing Seed and commitments on random input============================================================
  ...
  if(id==0){
      ...
      memcpy(buffer,&seed,sizeof(block)); //seed
      memcpy(buffer+sizeof(block),b_array,INPUT_4M/8); //b array
      memcpy(buffer+INPUT_4M/8+sizeof(block),&(commitP1P2[id][0]),SHA256_DIGEST_LENGTH); //commitment on random value
     // memcpy(buffer+INPUT_4M/8+sizeof(block)+SHA256_DIGEST_LENGTH,open12[id],sizeof(block));
  ...
  else{//receive randomness //Bob

    g_server_fd1 = socket_connect(ip,SERVER_PORT2);
    ...
    memcpy(cr12[id],buffer+sizeof(block)+INPUT_4M/8,SHA256_DIGEST_LENGTH); //send of that random commitment
  ...
  #ifdef DEBUG
    printf("Sent commitments! on random value\n");
  #endif
  //Sent Cs----------------------------------------------------------------------
...
int garble_handler(int id){
  ...
  int i,j;
  clock_t time_beg, time_end;
  char ccr1[2][SHA256_DIGEST_LENGTH], ccr2[2][SHA256_DIGEST_LENGTH];
  block oor1[2];
  ...
  else
  {
    ...
    //receive commitments on random value
    //dummy recv (for timing calculations)
    ...
    memcpy(&oor1[id],buffer+SHA256_DIGEST_LENGTH,sizeof(block));


    if(verify_commit(&(ccr1[id][0]),oor1[id],NULL,COMMIT_SCHEME_SHA256)== false)
      {
          cout<<"Commit not verified for "<<id<<"\n";
          exit(0);
      }
      printf("Received commitments on random values\n");
  ...
  //   memcpy(&commit_msg[j],buffer,SHA256_DIGEST_LENGTH);
  // }
  for(j=0;j< no_of_rounds;++j){
  ...
  #ifdef DEBUG
  printf("Received input commitments\n");
  #endif
  ...
  #ifdef DEBUG
  printf("Received random value commitment from %d\n",id );
  #endif
  ...
  time_beg = clock();
  memcpy(ccr2[id],buffer,SHA256_DIGEST_LENGTH);

  if(memcmp(&(ccr1[id][0]),&(ccr2[id][0]),SHA256_DIGEST_LENGTH) != 0){
      cout<<"commitment for C1&C2 are not equal\naborting..\n";
      // exit(0);
  }
  time_end = clock();
  comp_time_mtx.lock();
  ...
  if(id==1){//Bob_hanler will varify
    ...
    else if ((ccr2[0],ccr1[0],SHA256_DIGEST_LENGTH) != 0 || (ccr2[1],ccr1[1],SHA256_DIGEST_LENGTH) != 0)
    {
      cout<<"Commitments on random input mismatch";
    }
  ...
  memcpy(buffer+gc.m*sizeof(block),&oor1[(id+1)%2],sizeof(block));
  ...
  send(client_soc[id], buffer, gc.m*sizeof(block) + sizeof(block), 0);
